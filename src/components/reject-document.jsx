...
const RejectDocumentModal = ({ doc, onClose, onRejected }) => {
    ...
    const handleConfirm = async () => {
        ...
        try {
            const payload = {
                doc_status: "rejected",
                doc_reject_reason: reason,
                doc_rejected_by: user?.user_id,
            };
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50" onClick={onClose}>
            <div className="relative w-full max-w-md rounded-xl bg-white p-6 shadow-xl dark:bg-slate-900 dark:text-white" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="mb-6">
                    <label className="mb-2 block text-sm" htmlFor="reject-reason">Reason (optional)</label>
                    <textarea
                        id="reject-reason"
                ...
                <div className="flex justify-end gap-3">
                    <button onClick={onClose} className="rounded-lg bg-gray-200 px-4 py-2 text-sm font-medium hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600">Cancel</button>
                    <button onClick={handleConfirm} disabled={submitting} className={`rounded-lg px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:opacity-60`}>
