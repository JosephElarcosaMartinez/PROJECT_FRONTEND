...
const AddContact = ({ onAdd, onClose }) => {
  ...
  const handleChange = (e) => {
    ...
    if (
        !formData.clientContact_fullname ||
        !formData.clientContact_email ||
        !formData.clientContact_phonenum
    ) {
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex justify-end gap-2 mt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 bg-gray-300 dark:bg-slate-600 dark:text-white rounded"
            >
              Cancel
            </button>
            <button 
