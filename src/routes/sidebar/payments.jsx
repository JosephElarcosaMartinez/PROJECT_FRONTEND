...
export const Payments = () => {
  ...
  const filteredPayments = paymentsData.filter((payment) => {
    const matchesSearch = 
      ...
      payment.case.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  // Calculate totals
  const totalAmount = filteredPayments.reduce((sum, payment) => sum + payment.paid_amount, 0);
  const completedPayments = filteredPayments.filter(p => p.status === "Completed");
  const completedAmount = completedPayments.reduce((sum, payment) => sum + payment.paid_amount, 0);
      ...
      <div className="card w-full overflow-x-auto">
        <div className="h-[600px] overflow-y-auto">
          <table className="min-w-full table-auto text-left text-sm">
            <thead className="card-title text-xs uppercase sticky top-0 bg-white dark:bg-slate-900 z-10">
              <tr>
              ...
              paginatedPayments.map((payment) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="max-w-xs truncate" title={payment.case}>
                      {payment.case}
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3 font-bold text-green-600 dark:text-green-400">
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white p-8 shadow-lg dark:bg-slate-800">
            ...
            <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
              <div>
                ...
                <p className="text-gray-900 dark:text-slate-200">{viewPayment.reference_number}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-600 dark:text-blue-700">Status</p>
                <span
                  className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${getStatusBadge(viewPayment.status)}`}
                >
                  {viewPayment.status}
                </span>
              </div>
              <div>
                ...
                <p className="text-gray-900 dark:text-slate-200">{viewPayment.client}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-600 dark:text-blue-700">Case</p>
                <p className="text-gray-900 dark:text-slate-200">{viewPayment.case}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-gray-900 dark:text-slate-200">{viewPayment.payment_type}</p>
              </div>
              <div>
                <p className="font-semibold text-gray-600 dark:text-blue-700">Payment Method</p>
                <p className="text-gray-900 dark:text-slate-200">{viewPayment.payment_method}</p>
              </div>
            </div>
      ...
      {editPayment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-2xl rounded-xl bg-white p-8 shadow-lg dark:bg-slate-800">
            ...
            <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
              <div>
                <label className="font-semibold text-gray-600 dark:text-blue-700">Client</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-semibold text-gray-600 dark:text-blue-700">Case</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-semibold text-gray-600 dark:text-blue-700">Amount</label>
                <input
                  type="number"
                  step="0.01"
                  value={editPayment.paid_amount}
                  onChange={(e) => setEditPayment({ ...editPayment, paid_amount: parseFloat(e.target.value) })}
                  className="w-full rounded-md border px-3 py-2 text-slate-900 dark:bg-slate-700 dark:text-slate-50"
                />
              </div>
              <div>
                <label className="font-semibold text-gray-600 dark:text-blue-700">Date</label>
                <input
                  type="date"
              ...
              <div>
                <label className="font-semibold text-gray-600 dark:text-blue-700">Payment Type</label>
                <select
              ...
              <div>
                <label className="font-semibold text-gray-600 dark:text-blue-700">Status</label>
                <select
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <button
                onClick={() => {
                  // Here you would typically update the payment in your backend
